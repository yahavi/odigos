...
## Step 1: Identify the Helm Chart Version

The Odigos chart is published as an OCI artifact at `oci://odigosdemo.jfrog.io/helmoci/odigos`. Determine the desired `targetRevision` (chart version) you want to deploy.

## Step 2: Define the Argo CD Application Manifest
...
spec:
  project: default
  source:
    repoURL: odigosdemo.jfrog.io/helmoci
    chart: odigos
    targetRevision:  # REQUIRED: set the desired chart version
    helm:
      releaseName: odigos
      values: |
