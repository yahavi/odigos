...
```bash
helm repo update
helm upgrade --install odigos oci://odigosdemo.jfrog.io/helmoci/odigos --namespace odigos-system --create-namespace --set imagePrefix=$CUSTOM_DOCKER_REGISTRY --set onPremToken=$ODIGOS_TOKEN
